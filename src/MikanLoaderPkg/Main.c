...
EFI_STATUS EFIAPI UefiMain(
    ...
    if (EFI_ERROR(status)) {
        Print(L"Failezzzzzzzzzzzd to exit boot service (%r). Retry.\n", status);
        
        status = GetMemoryMap(&memmap);
        if (EFI_ERROR(status)) {
            Print(L"Failedaaaaaaaaaaaaaa to get memory map: %r\n", status);
            while (1);
        }
        status = gBS->ExitBootServices(image_handle, memmap.map_key);
        if (EFI_ERROR(status)) {
            Print(L"Faileddddddddddddd to exit boot service. Critical.\n");
            while (1);
        }
    }
